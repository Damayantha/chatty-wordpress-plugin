...
class Chatty_Widget_Plugin {
	...
	public function enqueue_widget_script() {
		...
		if ( 'include' === $settings['display_mode'] ) {
			...
			// phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict -- $pages are trimmed strings from a textarea; loose comparison intentionally matches "12" against the int ID get_queried_object_id() returns.
			if ( ! in_array( get_queried_object_id(), $pages, false ) && ! $this->url_matches_patterns( $pages ) ) {
				return;
			}
